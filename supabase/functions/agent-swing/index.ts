          ...
          if (!finalStopBoundCheck.valid) {
            ...
            if (solverResult.achievable) {
              ...
              const drilldownObj = {
                ...
                stop_plan_json: { stop: sl, initial: sl, atr: snapshot.atr_14 },
                take_profit_json: { tp: tp2, tp1, tp2, tp3 },
                ai_summary: `[SWING][${tier}][LTF Drilldown] ${safeRationale} | Routed to agent-day for tight M30 microstructure stop placement.`,
                ai_risks: `Routed to Lower Timeframe (LTF) Drilldown: Wide daily stop distance (${Math.abs(entry - sl).toFixed(3)}) routed to M30 drilldown for account equity protection.`,
                confidence: adjustedConfidence,
                trace_id: traceId,
              };
