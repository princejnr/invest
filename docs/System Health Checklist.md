...
3. **Resolution Deployed (2026-09-10):** `supabase/functions/agent-sniper/index.ts` was deployed as a lightweight decommission stub returning HTTP 200 `{ ok: true, status: "DECOMMISSIONED" }`, immediately eliminating all 1,440 daily 401 log errors.
...
## ⚠️ 1S. Stale PENDING_APPROVAL Signal Expiration & 24h Auto-Reconciliation Standard

> [!CAUTION]
> **Incident (2026-09-10):** 20 trade opportunities generated by `agent-swing` (with `isManual = true`) sat in `status = 'PENDING_APPROVAL'` for 1–4 days (from Sep 6 to Sep 9) without being reviewed or expired. Because `agent-sre` only checked for orphaned `PUBLISHED` and `APPROVED` signals, aged unreviewed signals remained permanently in limbo, creating a severe operational risk of executing stale entries if manually approved days later after market structure evolved.

### Diagnostic Protocol & Standard Rule:
1. Query `trade_opportunities` for aged `PENDING_APPROVAL` records older than 24 hours:
   ```sql
   ...
   ```
2. **Auto-Healing by Agent SRE (Probe 4B-2):** `agent-sre` automatically sweeps `trade_opportunities` every hour and transitions any `PENDING_APPROVAL` signal older than 24 hours to `status = 'EXPIRED'`, logging an auto-remediation record:
   ```sql
...
## ⚠️ 5B. MetaAPI History Polling Redundancy & Rate Limit (HTTP 429) Mitigation

> [!WARNING]
> **Incident (2026-09-10):** In `exness-history-sync`, background polling every 15 minutes queried MetaAPI `/history-deals` whenever open trades existed in `user_trades`. Because the zero-latency Windows VPS EA architecture is the primary source of truth (streaming live deal closures to `vps-history`), continuously querying MetaAPI for open positions generated recurring `HTTP 429 TooManyRequestsError: "It seems like you are trying to access too many unexisting or undeployed trading accounts..."` and `504 TimeoutError` warnings.

### Standard Architecture Rule:
1. **VPS-Primary Exclusivity:** When `user_risk_settings.vps_last_heartbeat` is active (< 5 minutes latency), `exness-history-sync` must bypass polling MetaAPI for open positions. The EA on the VPS calls `vps-history` immediately upon deal closure.
2. **Targeted Failover Sync:** `exness-history-sync` only queries MetaAPI when:
