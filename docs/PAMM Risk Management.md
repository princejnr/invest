...
| `trading_symbols` | **Asset Whitelist**: Array of active instruments approved for AI trade scanning. Calibrated to the high-conviction Pareto 80/20 universe: `BTCUSD`, `ETHUSD`, `XAUUSD`, `XAGUSD`, `USOIL`, `UKOIL`, `US30` (Forex pairs deactivated to eliminate negative-alpha churn and fee drag). |
...
| :--- | :--- | :--- |
| **Base/Quote Currency Conflict Guard** | [`packages/strategy/agent-risk.ts`](file:///Users/quagrained/workspace/raine/invest/packages/strategy/agent-risk.ts#L38-L200) | Decomposes forex and commodities into individual base/quote currency exposures. Blocks new orders that propose opposing directional exposure against heavily-aligned active currency clusters ($|\text{Net Exposure}| \ge 2$). |
| **Dynamic Correlation Limits** | [`supabase/functions/agent-trade/index.ts`](file:///Users/quagrained/workspace/raine/invest/supabase/functions/agent-trade/index.ts#L1641-L1680) | Categorizes assets into correlation groups (`USD`, `EQUITY_INDICES`, `PRECIOUS_METALS`, `ENERGY`). Blocks opposing trades on correlated pairs and reduces sizing by $50\%$ ($0.5\times$) if correlated exposure already exists. |
...
| **Dynamic ATR Stop Floor & Spread Buffers** | [`supabase/functions/agent-swing/index.ts`](file:///Users/quagrained/workspace/raine/invest/supabase/functions/agent-swing/index.ts) | Enforces minimum stop distances ($\ge 1.10\times\text{ATR}$ for day trades, $\ge 1.0\times\text{Daily ATR}$ for swing trades) plus asset-specific spread buffers to eliminate noise and spread hunting. |
...
| Guard | Code Reference | Description & Mechanism |
| :--- | :--- | :--- |
| **Dynamic Trailing Ladder** | [`supabase/functions/agent-trade/index.ts`](file:///Users/quagrained/workspace/raine/invest/supabase/functions/agent-trade/index.ts#L1160-L1204) | • **$\ge +0.35\text{R}$ Move or TP1 Touch**: Moves SL to **Friction-Buffered Break-Even** (+ spread + commissions).<br>• **$\ge +1.00\text{R}$ Move**: Locks in **$+0.50\text{R}$**.<br>• **$\ge +2.00\text{R}$ Move**: Locks in **$+1.00\text{R}$**.<br>• **$\ge +3.00\text{R}$ Move**: Locks in **$+2.00\text{R}$**.<br>• **Runner Leg**: Continually trailed at $1.5\times\text{ATR}$. |
| **Companion Breakeven Lock** | [`RaineInvestEA.mq5`](file:///Users/quagrained/workspace/raine/invest/packages/mql5-bridge/RaineInvestEA.mq5) & [`vps-history`](file:///Users/quagrained/workspace/raine/invest/supabase/functions/vps-history/index.ts) | When the companion `QUICK_EXIT` leg cashes out at Target 1, MT5 autonomously moves the remaining `RUNNER` SL to friction-buffered Break-Even locally in 0ms, and synchronizes `trade_opportunities.stop_plan_json`. |
...
| **End-of-Day Scalp Liquidation** | [`supabase/functions/agent-trade/index.ts`](file:///Users/quagrained/workspace/raine/invest/supabase/functions/agent-trade/index.ts#L1101-L1120) | Automatically liquidates cash-settled equities and index scalps at 4:00 PM NY time. 24-hour assets (Forex/Crypto) are protected from premature market dumping; if profitable, stop is advanced to Breakeven, otherwise allowed to run to target/stop. |
